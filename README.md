...
### ChatGPT document blocks in 0.6.9

ChatGPT may place generated documents inside an interactive writing-block editor between ordinary response paragraphs. The ChatGPT adapter recognizes semantic writing-block attributes, replaces each editor shell in its cloned response DOM with static semantic content, and retains a narrow internal document marker through sanitization. ReadBooster presents that content as a restrained **Document** block with a document-only Copy action in both Document and Focus modes. Headings, lists, tables, code, links, and emphasis retain their original response order and normal reader typography; inner code and table controls continue to work independently. ChatGPT edit, copy, fullscreen, and other host controls are excluded. Incomplete shells are skipped safely, and **Refresh conversation** can capture their completed content later. This support is specific to the verified ChatGPT structure and does not claim support for unrelated editable surfaces on Gemini or Mistral.
...
Version 0.6.9 is prepared as a build candidate, but this repository does not publish or submit it automatically. The production archive contains the contents of `dist-chrome/` at its root, so `manifest.json`, `icons/`, `assets/`, and `src/` are top-level ZIP entries rather than being nested under a build directory.
...
3. Run `npm run package:chrome` to create `release/readbooster-chrome-0.6.9.zip` with `manifest.json` at the archive root.
...
- Claude receives no production host permission, content script, adapter routing, or injected control in 0.6.9.
...
Later roadmap candidates include search, bookmarks, text-range anchoring, Sticker reattachment/export, response editing, AI-assisted revisions, selective print/export, and safe completion-triggered refresh. These features are not implemented in 0.6.9.
