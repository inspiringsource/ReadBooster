...
/** Finds the nearest meaningful scrollable ancestor shared by mounted source messages. */
...
export function waitForConversationDomToSettle(
...
export function createConversationScanSource(scroller: HTMLElement): ConversationScanSource {
  return {
    ...
    settle: (signal) => waitForConversationDomToSettle(scroller, signal),
    async restore(position) {
      setScrollPosition(scroller, position);
      try {
        await waitForConversationDomToSettle(scroller, undefined, {
          quietPeriodMs: 50,
          maximumWaitMs: 300,
        });
      } catch {
